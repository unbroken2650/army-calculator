...
function calc(event) {
    ...
    const result_1 = document.querySelector("#dischargeDay");
    result_1.innerHTML = `<p>예상 전역일은 <b>${dischargeDate.getFullYear()}년 ${Number(
        dischargeDate.getMonth() + 1
    )}월 ${dischargeDate.getDate()}일</b>이네요!</p>`;
    ...
    const totalDays = parseInt(
        (dischargeDate.getTime() - joinDate.getTime()) / (1000 * 60 * 60 * 24),
        10
    );
    const leftDays = parseInt(
        (dischargeDate.getTime() - currentDate.getTime()) /
            (1000 * 60 * 60 * 24),
        10
    );
    const leftPercent =
        parseInt(((totalDays - leftDays) / totalDays) * 10000, 10) / 100;

    // result_2
    if (leftPercent >= 100) {
        result_2.innerHTML = "<p>전역을 축하드립니다!</p>";
    } else if (leftPercent < 0) {
        result_2.innerHTML = "<p>입대 이전입니다.</p>";
    } else {
        result_2.innerHTML = `<p>${currentDate.getFullYear()}년 ${Number(
            currentDate.getMonth() + 1
        )}월 ${currentDate.getDate()}일을 기준으로 <b>${leftPercent}</b>% 지났습니다.</p>`;
    }

    // result_3
    result_3.innerHTML = `<p><b>월급 ${Math.floor(
        leftDays / 30
    )}번</b>만 더 받으면 전역! 미리 축하드립니다 :)</p>`;
